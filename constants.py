...
QUERY_CREATE_SELECT_PRODUCT = (
    "SELECT Product.id, Product.product_name, Product.category_name, Product.nutrition_grade, Product.product_url, Product.category_id from Product"
    " WHERE Product.id = %s"
)

QUERRY_BETTER_NUTRITION_GRADE = (
    "SELECT * FROM product" "WHERE nutrition_grade < %s" "AND category_id = %s"
)
